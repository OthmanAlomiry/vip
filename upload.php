...
if (isset($_POST["submit"]) && isset($_FILES['fileToUpload'])) {
    ...
    curl_setopt($ch, CURLOPT_USERAGENT, 'Mozilla/5.0 (Windows NT 10.0; Win64; x64)'); // تظاهر بأنك متصفح عادي
    ...
    if ($httpCode == 200 && strpos($response, 'https://') !== false) {
        ...
        echo "<input id='link' type='text' value='$downloadLink' style='width:100%; padding:15px; background:#000; color:#00ff00; border:1px solid #333; border-radius:8px; text-align:center;' readonly>";
        echo "<br><br>";
        echo "<a href='$downloadLink' target='_blank' style='display:inline-block; padding:12px 25px; background:#6200ee; color:white; text-decoration:none; border-radius:8px; font-weight:bold;'>فتح الملف</a>";
    } else {
        echo "<h1 style='color:#ff4444;'>❌ فشل الرفع</h1>";
        echo "<p>رد السيرفر: " . htmlspecialchars($response) . "</p>";
        echo "<p>كود الحالة: $httpCode</p>";
    }
